...
class FeatureExtractor():
    ...
    def MakeLifter(self):
        ...
        '''
        Q = self.lifter_coef
        EYE = np.arange(self.num_ceps)
        lifter = 1.0 + 0.5 * Q * np.sin(np.pi * EYE / Q)
        return lifter
...
if __name__ == "__main__":
    ...
    # train_out_dir = 'data/mfcc/train'
    train_out_dir = 'data/fbank/train'
    dev_wav_scp = 'data/dev/wav.scp'
    #  dev_out_dir = 'data/mfcc/dev'
    dev_out_dir = 'data/fbank/dev'
    test_wav_scp = 'data/eval/wav.scp'
    # test_out_dir = 'data/mfcc/eval'
    test_out_dir = 'data/fbank/eval'
            ...
            for line in file_wav:
                ...
                with wave.open(wav_path) as wav:
                    ...
                    if wav.getframerate() != sample_frequency:
                        sys.stderr.write(f"SR: {wav.getframerate()}")
                        sys.stderr.write(f"The expected sampling rate is {sample_frequency}.\n")
                        exit(1)
                    # wavファイルが1チャネル(モノラル)
                    # データであることをチェック
                    ...
                    # FBANKを計算する
                    feature, _ = feat_extractor.ComputeFBANK(waveform)

                    # # MFCCを計算する
                    # feature = feat_extractor.ComputeMFCC(waveform)

                # 特徴量のフレーム数と次元数を取得
                (num_frames, num_dims) = np.shape(feature)
