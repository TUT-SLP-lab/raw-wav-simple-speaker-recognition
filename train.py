...
if __name__ == "__main__":
    ...
    for mean_path, label_path, feat_path in zip(
        ...
        shutil.copyfile(mean_std_file,
                        os.path.join(output_dir, mean_path, 'mean_std.txt'))
    ...
    train_loader = DataLoader(datasets[0],
                              ...
                              shuffle=True,
                              num_workers=8)
    dev_loader = DataLoader(datasets[1],
                            batch_size=batch_size,
                            shuffle=False,
                            num_workers=8)
    eval_loader = DataLoader(datasets[2],
                             batch_size=batch_size,
                             shuffle=False,
                             num_workers=8)
    ...
    for epoch in range(max_num_epoch):
        ...
        for phase in ['train', 'valid']:
            ...
            for (features, labels, feat_len,
                 label_len, utt_ids) in dataset_loader[phase]:
