...
if __name__ == "__main__":
    ...
    for feature in feature_list:
        # 各特徴量ファイルのリストと
        # 平均・標準偏差計算結果の出力先
        train_feat_scp = 'data/%s/train/feats.scp' % (feature)
        train_out_dir = 'data/%s/train' % (feature)
        dev_feat_scp = 'data/%s/dev/feats.scp' % (feature)
        dev_out_dir = 'data/%s/dev' % (feature)
        eval_feat_scp = 'data/%s/eval/feats.scp' % (feature)
        eval_out_dir = 'data/%s/eval' % (feature)

        # 特徴量ファイルリストと出力先をリストにする
        feat_scp_list = [train_feat_scp, dev_feat_scp, eval_feat_scp]
        out_dir_list = [train_out_dir, dev_out_dir, eval_out_dir]
